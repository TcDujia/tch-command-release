...
if(fis.util.exists(hashTempPath)){
    oldHashData = fis.util.readJSON(hashTempPath);
}
...
function ftpUpload(dest, md5, release, content, file, callback){
    var output = dest.to,
        ftpConf = dest,
        serverConf = ftpConf.server;
    if(!serverConf){
        fis.log.error("ftp服务器的配置不存在！");
        return;
    }
    ...
    var config = merge(defaultConf,serverConf);
    var server = fis.config.data.server,
        path = fis.util.realpath(ftpConf.from);
    config.localRoot = path;
    config.remoteRoot= ftpConf.to;
...
function deploy(dest, file, callback){
    if(file.release){
        ...
        if(dest.receiver){
            ...
            if(dest.method === "ftp"){
                dest.to = dest.to.replace(dest.root,"");
                ftpUpload(dest, dest.opt.md5, release, content, file, callback);
            }else{
                //匹配打包的图片和脚本
                if(/\/\d+\.\d+\.\d+\/|\/img\/.*/.test(release)){
                    if(dest.opt.md5){
                        var hash = file.getHash();
                        if(oldHashData && oldHashData[release] !== hash || !oldHashData){
                            //如果是匹配图片
                            var _diffRelease;
                            if(/\/img\/.*/.test(release)){
                                if(release.indexOf("_"+hash)===-1){
                                    _diffRelease = release.replace(/\.\w+$/,function($0){
                                        return "_"+hash+$0;
                                    })
                                }
                            }
                            diffHashTemp.push(_diffRelease||release);
                        }
                        hashTemp[release] = hash;
                        if(hashT) {
                            clearTimeout(hashT);
                        }
                        hashT = setTimeout(function(){
                            fis.util.write(diffHashPath,JSON.stringify(diffHashTemp));
                            fis.util.write(hashTempPath,JSON.stringify(hashTemp));
                        },200);
                    }
                }else{
                    hashTemp[release] = hash;
...
var exports = module.exports = function(opt, collection,diffCollection){
    var settings = fis.config.get('deploy', {});
    var root = fis.project.getProjectPath();
    var dests = [],
        dest = opt.dest;
    if(typeof dest === "string"){
        fis.log.error("目录配置已经修改,请调整tch-conf里的配置");
        return;
    }
    if(!dest.to){
        if(
    ...
    fis.util.map(collection, function(subpath, file){
        ...
        dests.forEach(function(d){
            var from = d.from;
            if(from.indexOf("output")>-1){
                from = from.split("output")[1];
            };
            if(file.release &&
                file.release.indexOf(from) === 0 &&   //relate to replaceFrom
